...
def _install_fakes():
    ...
    fake_data = sys.modules.get("vnstock_data")
    ...
    # Another offline test module may have installed a smaller fake first.
    # Fill the complete contract needed by main.py instead of depending on
    ...
    if "vnstock_data.explorer.kbs" not in sys.modules:
    ...
    for submod in ("company", "listing"):
        module_name = f"vnstock_data.explorer.kbs.{submod}"
    ...
    if "vnstock_data.explorer.vci" not in sys.modules:
        fake_vci = types.ModuleType("vnstock_data.explorer.vci")
        fake_vci.__path__ = []
        sys.modules["vnstock_data.explorer.vci"] = fake_vci
    if "vnstock_data.explorer.vci.company" not in sys.modules:
